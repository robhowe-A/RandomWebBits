...
export class ToDoList {
  ...
  public createToDoListWidget(elem: Element) {
    ...
    if (elem == undefined) {
      console.log(
        `%cThere is no "ToDoList" class on this page.`,
        "color:orange;"
      );
      return;
    }
    ...
    switch (window.location.pathname) {
      ...
      case "/dist/index.html":
        ...
        const todolistSection = elem.insertAdjacentElement(
          "afterend",
          document.createElement("section")
        );
        const header = todolistSection.appendChild(
  ...
  private addtoDoToStorage(description: string) {
    ...
    console.log(
      `%c<RWB>%cAdded to-do cache: ${description}`,
  ...
  private removetoDoFromStorage(item: string) {
    ToDoList.ToDoInStorage = ToDoList.ToDoInStorage.filter(
      todo => todo.todoitem !== item
    );
    console.log(
    ...
    let todoinstoragestrgfytest = Object.create(
      new RWBStringifyJSON(ToDoList.ToDoInStorage)
    );
  ...
  private AddToDoRow(description: string, firstPaint: boolean) {
    ...
    newITEM.setAttribute(
      ...
      ToDoList.ToDOs
        ? (() => {
            ...
            return (
              (Number(elem?.getAttribute("num")) || -1000) + ToDoList.ToDOs
            ).toString();
          })()
        : (1).toString()
    );
    ...
    console.log(
      `%c<RWB>%cCreated to-do table row`,
  ...
  private DeleteButton(box: HTMLInputElement) {
    ...
    const rowChkBx = <HTMLElement>(
      box.parentNode.previousSibling.previousSibling
    );
    /** Input element */
